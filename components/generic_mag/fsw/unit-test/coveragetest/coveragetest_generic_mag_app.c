...
/*
** File: coveragetest_generic_mag_app.c
**
** Purpose:
** Coverage Unit Test cases for the GENERIC_mag Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the GENERIC_mag application.
**
...
#include "generic_mag_app_coveragetest_common.h"
#include "ut_generic_mag_app.h"

/* to get the GENERIC_mag_LIB_Function() declaration */
...
void Test_GENERIC_mag_AppMain(void)
{
    ...
    /*
     * GENERIC_mag_AppMain does not return a value,
     ...
     * The first is for GENERIC_mag_AppInit().  As this is in the same
     ...
     * the GENERIC_mag_AppInit() failure path was taken.
     */
    GENERIC_mag_AppMain();
     ...
     */
    UtAssert_True(GENERIC_mag_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "GENERIC_mag_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)GENERIC_mag_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_PIPE_ERR_EID, "GENERIC_mag: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_GENERIC_mag_AppInit(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(GENERIC_mag_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(GENERIC_mag_AppInit(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(GENERIC_mag_AppInit(), CFE_SB_BAD_ARGUMENT);
...
void Test_GENERIC_mag_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void GENERIC_mag_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t          SBBuf;
        GENERIC_mag_NoArgs_cmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_PROCESS_CMD_ERR_EID, NULL);

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(GENERIC_mag_CMD_MID);
    FcnCode   = GENERIC_mag_NOOP_CC;
    ...
    UtAssert_True(EventTest.MatchCount == 0, "GENERIC_mag_CMD_ERR_EID not generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    GENERIC_mag_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_GENERIC_mag_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void GENERIC_mag_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(GENERIC_mag_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t          SBBuf;
        GENERIC_mag_NoArgs_cmd_t Noop;
        GENERIC_mag_NoArgs_cmd_t Reset;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_CMD_NOOP_INF_EID, NULL);
    GENERIC_mag_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = GENERIC_mag_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_CMD_RESET_INF_EID, NULL);
    GENERIC_mag_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_CMD_ERR_EID, NULL);
    GENERIC_mag_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_GENERIC_mag_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void GENERIC_mag_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(GENERIC_mag_REQ_HK_TLM);

    /* Set message id to return so GENERIC_mag_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    UtAssert_True(MsgSend == &GENERIC_mag_AppData.HkTelemetryPkt.TlmHeader.Msg,
    ...
    UtAssert_True(MsgTimestamp == &GENERIC_mag_AppData.HkTelemetryPkt.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");
}

void Test_GENERIC_mag_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool GENERIC_mag_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(GENERIC_mag_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_LEN_ERR_EID, NULL);

    GENERIC_mag_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "GENERIC_mag_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_mag_LEN_ERR_EID, NULL);
    GENERIC_mag_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_mag_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
 ...
 */
void Generic_mag_UT_Setup(void)
{
    UT_ResetState(0);
}
 ...
 */
void Generic_mag_UT_TearDown(void) {}
...
{
    ADD_TEST(GENERIC_mag_AppMain);
    ADD_TEST(GENERIC_mag_AppInit);
    ADD_TEST(GENERIC_mag_ProcessCommandPacket);
    ADD_TEST(GENERIC_mag_ProcessGroundCommand);
    ADD_TEST(GENERIC_mag_ReportHousekeeping);
    ADD_TEST(GENERIC_mag_VerifyCmdLength);
}
