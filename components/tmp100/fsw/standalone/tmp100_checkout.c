    ...
    switch (cc)
    {
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                ...
                if (status == OS_SUCCESS)
                {
                    OS_printf("TMP100_RequestHK command success\n");
                }
                else
                {
                    OS_printf("TMP100_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_TMP100:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = TMP100_RequestData(&Tmp100Uart, &Tmp100Data);
                if (status == OS_SUCCESS)
                {
                    OS_printf("TMP100_RequestData command success\n");
                }
                else
                {
                    OS_printf("TMP100_RequestData command failed!\n");
                }
            }
            break;
...
/* Initialize HWLIB */
    ...
    // Close the device
    uart_close_port(&Tmp100Uart);

#ifdef _NOS_ENGINE_LINK_
    nos_destroy_link();
#endif

    OS_printf("Cleanly exiting tmp100 application...\n\n");
    return 1;
}
