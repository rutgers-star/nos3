...
** Include Files
*/
#include "raspberry_pi_checkout.h"
...
void print_help(void)
{
    printf(PROMPT "command [args]\n"
                  ...
                  "  h                                - ^                               \n"
                  "raspberry_pi                             - Request raspberry_pi data             \n"
                  "  s                                - ^                               \n"
...
int get_command(const char *str)
{
    ...
    else if (strcmp(lcmd, "raspberry_pi") == 0)
    {
        status = CMD_RASPBERRY_PI;
    }
    ...
    switch (cc)
    {
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                ...
                if (status == OS_SUCCESS)
                {
                    OS_printf("RASPBERRY_PI_RequestHK command success\n");
                }
                else
                {
                    OS_printf("RASPBERRY_PI_RequestHK command failed!\n");
                }
            }
            break;

        case CMD_RASPBERRY_PI:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = RASPBERRY_PI_RequestData(&Raspberry_piUart, &Raspberry_piData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("RASPBERRY_PI_RequestData command success\n");
                }
                else
                {
                    OS_printf("RASPBERRY_PI_RequestData command failed!\n");
                }
            }
            break;
...
/* Initialize HWLIB */
    ...
    // Close the device
    uart_close_port(&Raspberry_piUart);

#ifdef _NOS_ENGINE_LINK_
    nos_destroy_link();
#endif

    OS_printf("Cleanly exiting raspberry_pi application...\n\n");
    return 1;
}
